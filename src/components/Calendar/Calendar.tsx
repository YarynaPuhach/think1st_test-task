      ...
      <div className="calendar-header">
        ...
        >
          <svg width="11" height="14" viewBox="0 0 11 14" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0.499999 7.86602C-0.166668 7.48112 -0.166667 6.51888 0.5 6.13397L9.5 0.937821C10.1667 0.552921 11 1.03405 11 1.80385L11 12.1962C11 12.966 10.1667 13.4471 9.5 13.0622L0.499999 7.86602Z" fill="#CBB6E5" />
          </svg>
        ...
        >
          <svg width="11" height="14" viewBox="0 0 11 14" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10.5 7.86602C11.1667 7.48112 11.1667 6.51888 10.5 6.13397L1.5 0.937821C0.833334 0.552921 6.10471e-07 1.03405 5.76822e-07 1.80385L1.2256e-07 12.1962C8.8911e-08 12.966 0.833333 13.4471 1.5 13.0622L10.5 7.86602Z" fill="#CBB6E5" />
          </svg>
        </button>
      </div>
    );
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          <div
            key={formattedDate}
            className={`day-cell ${!isInCurrentMonth ? 'empty' :
              isSelected ? 'selected' :
                isDisabled ? 'holiday' : ''
              }`}
            onClick={() => {
              if (isInCurrentMonth && !isDisabled) {
                onDateChange(formattedDate);
                setMessage('');
              } else if (!isInCurrentMonth) {
                // Optionally handle click on empty cells if needed
              } else {
                const holidayName = getHolidayName(formattedDate);
                setMessage(`It is ${holidayName || 'a non-working day'} today`);
              }
            }}
          >
